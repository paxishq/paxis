...
async function upsertSupplier(id: string, name: string, vatNumber: string, country: string) {
	await db
		.insert(suppliers)
	...
	await db.insert(aiInventories).values([
		{
			...
			description: "Demand forecasting for inventory management — no automated decision-making affecting individuals",
			riskTier: "minimal",
			justification: "Used solely for supply chain optimisation; no Annex III use case.",
		},
	]);
}
...
console.log("  Suppliers: BoltCo (completed), SteelWorks (in_progress), GreenPackaging (sent), LogiTrans (overdue)");
console.log("  Scope 3: 232 tCO₂e from 1 completed supplier (25% completion rate)");
