...
export async function runIntake(
	...
	if (!questionnaireId || !supplierId) {
		...
		return { status: "skipped", reason: "missing questionnaireId or supplierId" };
	}
...
Map as many questions as possible from this data. Return JSON only.`.trim();
	...
	try {
		...
		if (!parsed.success) {
			throw new Error(`Intake LLM returned invalid mapping: ${parsed.error.message}`);
		}
	...
	if (mapping.suggestions.length > 0) {
		...
		if (existing) {
			await db
				...
				.set({ answers: { ...existing.answers as Record<string, unknown>, ...autoAnswers }, updatedAt: new Date() })
