...
export async function runAiAct(
	...
	if (items.length === 0) {
		...
		return { status: "ok", classifiedCount: 0, message: "No AI inventory items to classify" };
	}

	let classifiedCount = 0;
	const classifications: Array<{ id: string; toolName: string; riskTier: string }> = [];
	const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

	for (const item of items) {
		// Skip items reviewed within the last 30 days
		if (item.reviewedAt && item.justification && item.reviewedAt > thirtyDaysAgo) {
			classifications.push({ id: item.id, toolName: item.toolName, riskTier: item.riskTier });
			continue;
		}
...
Classify this system according to the EU AI Act and provide documentation requirements.`;
		...
		try {
			...
			const parsed = AiActClassificationSchema.safeParse(JSON.parse(extractJson(raw)));

			if (parsed.success) {
				const { riskTier, justification, keyRisks, documentationRequired } = parsed.data;
				await db
				...
				classifications.push({ id: item.id, toolName: item.toolName, riskTier });
				classifiedCount++;
			}
		} catch {
	...
	return { status: "ok", totalItems: items.length, classifiedCount, classifications };
}
