...
const RISK: Record<string, { label: string; chip: string }> = {
  ...
  high: {
    label: "High",
    chip: "bg-orange-950/50 text-orange-400 border-orange-900/50",
  },
  limited: {
    label: "Limited",
    chip: "bg-amber-950/50 text-amber-400 border-amber-900/50",
...
function RiskChip({ tier }: { tier: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium border ${cfg.chip}`}
    >
      {cfg.label}
    </span>
  );
  ...
  return (
    <div className="rounded-lg bg-white/[0.025] border border-white/[0.06] px-5 py-4">
      ...
      <p className="font-mono text-[26px] font-medium text-white leading-none tabular-nums">
        ...
        {unit && (
          <span className="text-zinc-600 text-sm font-normal ml-1.5">
            {unit}
          </span>
        )}
      </p>
...
function UploadDocumentDialog({ onParsed }: { onParsed: () => void }) {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/supplier/carbon/parse", {
      method: "POST",
      body: formData,
    });
  ...
  const confidenceCfg = result?.confidence
    ? CONFIDENCE_CFG[result.confidence]
    : null;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button
          size="sm"
          variant="outline"
          className="h-8 gap-1.5 text-[13px] border-emerald-900/50 text-emerald-500 hover:bg-emerald-950/30"
        >
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <p className="text-[12px] text-zinc-600 mt-0.5">
            Upload an energy bill, utility invoice, or fuel receipt — Gemini
            will extract Scope 1 &amp; 2 figures automatically.
          </p>
        </DialogHeader>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[13px] font-medium text-emerald-400">
                    {result.entriesInserted} entr
                    {result.entriesInserted === 1 ? "y" : "ies"} extracted
                  </span>
                </div>
                {confidenceCfg && (
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded border ${confidenceCfg.bg} ${confidenceCfg.color}`}
                  >
                    {confidenceCfg.label}
                  </span>
                )}
          ...
          <div className="space-y-3 pt-1">
            <div className="rounded-lg border border-red-900/50 bg-red-950/30 px-4 py-3.5 flex items-start gap-2">
              ...
              <p className="text-[12px] text-red-300 leading-relaxed">
                {result.error ?? "Parsing failed. Try a clearer image or PDF."}
              </p>
            </div>
            <DialogFooter>
              <Button
...
function RegisterAiDialog({
  onAdded,
}: {
  onAdded: (item: AiInventory) => void;
}) {
  const [open, setOpen] = useState(false);
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 pt-1">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={riskTier}
              onValueChange={(v) => setRiskTier(v as AiInventory["riskTier"])}
            >
              <SelectTrigger className="h-8 text-sm">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="minimal" className="text-sm">
                  Minimal risk
                </SelectItem>
                <SelectItem value="limited" className="text-sm">
                  Limited risk
                </SelectItem>
                <SelectItem value="high" className="text-sm">
                  High risk
                </SelectItem>
                <SelectItem value="unacceptable" className="text-sm">
                  Unacceptable risk
                </SelectItem>
              </SelectContent>
            </Select>
...
function ComplianceReportPage({
  ...
  const scope1Total = carbonEntries
    .filter((e) => e.scope === "scope1")
    .reduce((s, e) => s + e.co2Tonnes, 0);
  const scope2Total = carbonEntries
  ...
  const hasHighRisk = aiInventory.some(
    (a) => a.riskTier === "unacceptable" || a.riskTier === "high",
  );
  const aiByTier = {
    unacceptable: aiInventory.filter((a) => a.riskTier === "unacceptable")
      .length,
  ...
  const completedQ = questionnaires.filter(
    (q) => q.status === "completed",
  ).length;
  const pendingQ = questionnaires.filter(
  ...
  const checks = [
    {
      ...
      detail: hasScope1
        ? `${scope1Total.toLocaleString()} tCO₂e recorded`
    ...
    },
    {
      ...
      detail: hasScope2
        ? `${scope2Total.toLocaleString()} tCO₂e recorded`
        : "Add at least one Scope 2 entry",
    },
    {
      label: "AI inventory registered",
      done: aiInventory.length > 0,
      detail:
        aiInventory.length > 0
          ? `${aiInventory.length} system${aiInventory.length !== 1 ? "s" : ""} registered`
          : "Register any AI tools your org uses",
    },
    {
      label: "No unacceptable or high-risk AI",
      done: !hasHighRisk,
      detail: !hasHighRisk
        ? "All systems within acceptable tiers"
        : `${aiByTier.unacceptable + aiByTier.high} system(s) need review`,
    },
    {
      label: "All questionnaires responded",
      done: totalQ > 0 && allResponded,
      detail:
        totalQ === 0
          ? "No questionnaires received yet"
          : allResponded
            ? `${completedQ} of ${totalQ} completed`
            : `${pendingQ} pending, ${overdueQ} overdue`,
    },
  ];
    ...
    <div>
      ...
      <div className="mb-5 rounded-lg border border-white/[0.06] bg-white/[0.02] px-5 py-4 flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <span
              className={`text-[11px] font-medium px-2 py-0.5 rounded border ${readinessCfg.bg} ${readinessCfg.color}`}
            >
              {readinessCfg.label}
            </span>
        ...
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.015] divide-y divide-white/[0.04]">
          {checks.map((check) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span
                  className={`text-[13px] font-medium ${check.done ? "text-zinc-300" : "text-zinc-500"}`}
                >
                  {check.label}
                </span>
              </div>
              <span
                className={`text-[11px] shrink-0 ${check.done ? "text-zinc-600" : "text-amber-500/80"}`}
              >
                {check.detail}
              </span>
            </div>
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.015] divide-y divide-white/[0.04]">
            {(["scope1", "scope2"] as const).map((scope) => {
              ...
              const label =
                scope === "scope1"
                  ...
                  : "Scope 2 — Purchased energy";
              const color =
                scope === "scope1" ? "text-blue-400" : "text-purple-400";
              return (
                <div
                  key={scope}
                ...
                >
                  ...
                  {has ? (
                    <span className="font-mono text-[13px] text-white tabular-nums">
                      ...
                      <span className="text-zinc-600 text-[11px]">tCO₂e</span>
                    </span>
                  ) : (
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-white/[0.015]">
              ...
              <span className="font-mono text-[13px] font-medium text-white tabular-nums">
                {(scope1Total + scope2Total).toLocaleString()}{" "}
                <span className="text-zinc-600 text-[11px]">tCO₂e</span>
              </span>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.015] divide-y divide-white/[0.04]">
            {(["unacceptable", "high", "limited", "minimal"] as const).map(
              (tier) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`font-mono text-[13px] ${count > 0 ? "text-white" : "text-zinc-700"}`}
                    >
                      {count}
                    </span>
                  </div>
              ...
              {[
                {
                  ...
                  value: completedQ,
                  color:
                    "bg-emerald-950/60 text-emerald-400 border-r border-white/[0.04]",
                },
                {
                  label: "Pending",
                  value: pendingQ,
                  color:
                    "bg-amber-950/40 text-amber-400 border-r border-white/[0.04]",
                },
                {
                  label: "Overdue",
                  value: overdueQ,
                  color:
                    "bg-red-950/40 text-red-400 border-r border-white/[0.04]",
                },
                { label: "Total", value: totalQ, color: "text-zinc-300" },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className={`flex-1 px-4 py-3 ${stat.color}`}
                >
                  <p className="text-[10px] font-medium opacity-70 uppercase tracking-wide mb-1">
                    {stat.label}
                  </p>
                  <p className="font-mono text-[20px] font-medium leading-none">
                    {stat.value}
                  </p>
                </div>
              ))}
...
export default function SupplierApp() {
  ...
  function reloadCarbon() {
    fetch("/api/supplier/carbon")
      .then((r) => (r.ok ? r.json() : []))
      .then(setCarbonEntries);
  }
  ...
  useEffect(() => {
    ...
    fetch("/api/supplier/carbon")
      .then((r) => (r.ok ? r.json() : []))
      .then(setCarbonEntries);
    fetch("/api/supplier/ai-inventory")
      .then((r) => (r.ok ? r.json() : []))
      .then(setAiInventory);
  }, []);
          ...
          <div>
            <PageHeader
              ...
              action={
                <div className="flex items-center gap-2">
                  ...
                  <AddCarbonDialog
                    onAdded={(e) => setCarbonEntries((prev) => [...prev, e])}
                  />
                </div>
              }
            />
            ...
            {carbonEntries.length === 0 ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {["Scope", "CO₂e (tonnes)", "Period", "Source", ""].map(
                      (h) => (
                        <th
                          key={h}
                          className="pb-2.5 text-left text-[10px] font-medium text-zinc-600 uppercase tracking-[0.08em]"
        ...
        {page === "ai-inventory" && !selectedAiItemId && (
          <div>
            <PageHeader
              ...
              action={
                <RegisterAiDialog
                  onAdded={(item) => setAiInventory((prev) => [...prev, item])}
                />
              }
            />
            {aiInventory.length === 0 ? (
