...
import { Building2, FileText, LayoutGrid, Plus, Send } from "lucide-react";
...
export default function EnterpriseApp() {
	...
	useEffect(() => {
		fetch("/api/enterprise/me").then((r) => r.json()).then(setEnterprise);
		fetch("/api/enterprise/suppliers").then((r) => r.json()).then(setSuppliers);
		fetch("/api/enterprise/questionnaires").then((r) => r.json()).then(setQuestionnaires);
	}, []);
